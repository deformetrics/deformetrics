#'ontrajplot
#' This function plots in 2D or 3D, with or without heatmap, the ontogenetic trajectories of data transported according to the Linear Shift procedure implemented in ptau6() function. It takes a ptau6 object and use it to plot per-group predictions evaluated at user-defined equally spaced size values within per-group size ranges. The user can choose to plot landmarks only, landmarks with links, deformation grids (for 2D data) only, heatmap+deformation grids (for 2D data), heatmap in 3D or just mesh with triangulation in 3D. The heatmap is calculated according to Marquez et al (2012) [Márquez, E.J.; Cabeen, R.; Woods, R.P.; Houle, D. 2012. The measurement of local variation in shape. Evolutionary Biology 39:419-439]. There the log(determinant) of the jacobian matrix evaluated locally on the TPS function estimated between source and target is taken as representative of local deformation. In 3D the TPS is evaluated on the planes identified by single triangles and for this reason the first derivative of TPS is evaluated still in 2 dimensions. A full three dimensional evaluation requires a tetrahedralized shape. Values smaller than 0 indicate a restriction of the TPS grid'cells, while those larger than 0 indicate an enlargement. In 2D, if links are provided, the heatmap is calculated on the Delaunay triangulation constrained by the links. However, the links should identify, among the other structures, a closed contour. In 3D the Delaunay triangulation is performed within any single triangle. Two plots are produced: one for predictions transported via Linear Shift and one for original per-group-predictions. In the first case the first (undeformed) shape is common to all groups. As for complex datasets (see examples) the calculation could be slow, the function outputs an object that can be re-plotted rapidly using the function replotontraj().
